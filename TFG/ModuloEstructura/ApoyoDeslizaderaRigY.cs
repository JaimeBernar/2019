using OpenTK;
using OpenTK.Graphics.OpenGL;
using System;

namespace TFG
{
    [Serializable]
    class ApoyoDeslizaderaRigY : Apoyo
    {
        public ApoyoDeslizaderaRigY(double x, double y)
        {
            X = x;
            Y = y;
        }
        internal override void Dibujar2D(double TAM, double RR, double GG, double BB)
        {
            int increm = 20;
            GL.PushMatrix();
            GL.Translate(X, Y, 0);
            GL.Rotate(90, 0, 0, 1);
            GL.PushMatrix();
            Rectangulo rect = new Rectangulo(0.5 * TAM, TAM, 0, 0, Theta + 90);
            rect.Dibujar(RR, GG, BB);
            GL.End();
            GL.PopMatrix();

            GL.PushMatrix();
            GL.Translate(0, 0 - 0.4 * TAM, 0);
            GL.Begin(PrimitiveType.TriangleFan);
            GL.Color3(RR, GG, BB);
            for (double i = 0; i < 360; i += increm)
            {
                Vector3d Vect1 = new Vector3d(TAM / 6.0 * Math.Cos(i * (Math.PI / 180)), TAM / 6.0 * Math.Sin(i * (Math.PI / 180)), 0);
                Vector3d Vect2 = new Vector3d(TAM / 6.0 * Math.Cos((i) * (Math.PI / 180) + increm * Math.PI / 180), TAM / 6.0 * Math.Sin((i) * (Math.PI / 180) + increm * Math.PI / 180), 0);
                Vector3d Vect3 = new Vector3d(0, 0, 0);
                GL.Vertex3(Vect1.X, Vect1.Y, Vect1.Z);
                GL.Vertex3(Vect2.X, Vect2.Y, Vect2.Z);
                GL.Vertex3(Vect3.X, Vect3.Y, Vect3.Z);
            }
            GL.End();
            GL.PopMatrix();

            GL.PushMatrix();
            GL.Translate(0 - 0.35 * TAM, 0 - 0.4 * TAM, 0);
            GL.Rotate(Theta, 0, 0, 1);
            GL.Begin(PrimitiveType.TriangleFan);
            GL.Color3(RR, GG, BB);
            for (double i = 0; i < 360; i += increm)
            {
                Vector3d Vect1 = new Vector3d(TAM / 6.0 * Math.Cos(i * (Math.PI / 180)), TAM / 6.0 * Math.Sin(i * (Math.PI / 180)), 0);
                Vector3d Vect2 = new Vector3d(TAM / 6.0 * Math.Cos((i) * (Math.PI / 180) + increm * Math.PI / 180), TAM / 6.0 * Math.Sin((i) * (Math.PI / 180) + increm * Math.PI / 180), 0);
                Vector3d Vect3 = new Vector3d(0, 0, 0);
                GL.Vertex3(Vect1.X, Vect1.Y, Vect1.Z);
                GL.Vertex3(Vect2.X, Vect2.Y, Vect2.Z);
                GL.Vertex3(Vect3.X, Vect3.Y, Vect3.Z);
            }
            GL.End();
            GL.PopMatrix();

            GL.PushMatrix();
            GL.Translate(+0.35 * TAM, -0.4 * TAM, 0);
            GL.Begin(PrimitiveType.TriangleFan);
            GL.Color3(RR, GG, BB);
            for (double i = 0; i < 360; i += increm)
            {
                Vector3d Vect1 = new Vector3d(TAM / 6.0 * Math.Cos(i * (Math.PI / 180)), TAM / 6.0 * Math.Sin(i * (Math.PI / 180)), 0);
                Vector3d Vect2 = new Vector3d(TAM / 6.0 * Math.Cos((i) * (Math.PI / 180) + increm * Math.PI / 180), TAM / 6.0 * Math.Sin((i) * (Math.PI / 180) + increm * Math.PI / 180), 0);
                Vector3d Vect3 = new Vector3d(0, 0, 0);
                GL.Vertex3(Vect1.X, Vect1.Y, Vect1.Z);
                GL.Vertex3(Vect2.X, Vect2.Y, Vect2.Z);
                GL.Vertex3(Vect3.X, Vect3.Y, Vect3.Z);
            }
            GL.End();
            GL.PopMatrix();
            GL.PopMatrix();
        }
    }
}
