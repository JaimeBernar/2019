...
{
    [Serializable]
    internal class BarraRigidaArticulada : Barra
    {
        public BarraRigidaArticulada(Vector3d Inicio, Vector3d Final)
        {
            X0 = Inicio.X;
            Y0 = Inicio.Y;
            X1 = Final.X;
        ...
        internal override void MatrizRigidez()
        {
            ...
            //Submatrices en ejes globales
            {
                ...
                //MATRIZ K11
                Kii[0, 0] = ((E * A * c * c) / L) + (3 * E * I * s * s) / (L * L * L);
                Kii[0, 1] = ((E * A * s * c) / L) - (3 * E * I * s * c) / (L * L * L);
                Kii[0, 2] = (-3 * E * I * s) / (L * L);
                Kii[1, 0] = (E * A * s * c) / L - (3 * E * I * s * c) / (L * L * L);
                Kii[1, 1] = (E * A * s * s) / L + (3 * E * I * c * c) / (L * L * L);
                Kii[1, 2] = (3 * E * I * c) / (L * L);
                Kii[2, 0] = (-3 * E * I * s) / (L * L);
                Kii[2, 1] = (3 * E * I) / (L * L);
                Kii[2, 2] = (3 * E * I) / L;
                ...
                //MATRIZ K12
                Kij[0, 0] = ((-E * A * c * c) / L) - (3 * E * I * s * s) / (L * L * L);
                Kij[0, 1] = ((-E * A * s * c) / L) + (3 * E * I * s * c) / (L * L * L);
                Kij[0, 2] = 0;
                Kij[1, 0] = ((-E * A * s * c) / L) + (3 * E * I * s * c) / (L * L * L);
                Kij[1, 1] = ((-E * A * s * s) / L) - (3 * E * I * c * c) / (L * L * L);
                Kij[1, 2] = 0;
                Kij[2, 0] = (3 * E * I * s) / (L * L);
                Kij[2, 1] = (-3 * E * I * c) / (L * L);
                Kij[2, 2] = 0;
                ...
                //MATRIZ K21
                Kji[0, 0] = ((-E * A * c * c) / L) - (3 * E * I * s * s) / (L * L * L);
                Kji[0, 1] = ((-E * A * s * c) / L) + (3 * E * I * s * c) / (L * L * L);
                Kji[0, 2] = (3 * E * I * s) / (L * L);
                Kji[1, 0] = ((-E * A * s * c) / L) + (3 * E * I * s * c) / (L * L * L);
                Kji[1, 1] = ((-E * A * s * s) / L) - (3 * E * I * c * c) / (L * L * L);
                Kji[1, 2] = (-3 * E * I * c) / (L * L);
                Kji[2, 0] = 0;
                Kji[2, 1] = 0;
                Kji[2, 2] = 0;
                ...
                //MATRIZ K22
                Kjj[0, 0] = ((E * A * c * c) / L) + (3 * E * I * s * s) / (L * L * L);
                Kjj[0, 1] = ((E * A * s * c) / L) - (3 * E * I * s * c) / (L * L * L);
                Kjj[0, 2] = 0;
                Kjj[1, 0] = ((E * A * s * c) / L) - (3 * E * I * s * c) / (L * L * L);
                Kjj[1, 1] = ((E * A * s * s) / L) + (3 * E * I * c * c) / (L * L * L);
                Kjj[1, 2] = 0;
                Kjj[2, 0] = 0;
                Kjj[2, 1] = 0;
                Kjj[2, 2] = 0;
        ...
        internal override void DeformacionesFase0(Carga carga)
        {
            ...
            if (carga.GetType() == typeof(CargaDistribuida))
            {
                ...
                double Mb = -L * L * (7.0 * Q1YL + 8.0 * Q2YL) / 120.0;
                double Ra = L * (2.0 * Q1YL + Q2YL) / 6.0 + Mb / L;
                double Rb = L * (Q1YL + 2.0 * Q2YL) / 6.0 - Mb / L;
                ...
                for (int i = 0; i < ListaFase0.Count; i++)
                {
                    ...
                    //Deformacion lateral
                    v = x / (120.0 * L * EI) * ((Q2YL - Q1YL) * x * x * x * x + 5.0 * L * Q1YL * x * x * x - 20.0 * Ra * L * x * x + 5.0 * L * (12.0 * Ra * L * L - (3.0 * Q1YL + Q2YL) * L * L * L));
                    ListaFase0[ListaFase0.Count - 1 - i].Dy = v;
                }
            }
        ...
        internal override void EsfuerzosFase0(Carga carga)
        {
            ...
            if (carga.GetType() == typeof(CargaDistribuida))
            {
                ...
                double Xa = -(2.0 * Q1XL + Q2XL) * Longitud / 6.0;
                double Xb = -(Q1XL + 2.0 * Q2XL) * Longitud / 6.0;
                double Ma = -(8.0 * Q1YL + 7.0 * Q2YL) * Longitud * Longitud / 120.0;
                double Ra = -(48.0 * Q1YL + 27.0 * Q2YL) * Longitud / 120.0;
                double Rb = -(12.0 * Q1YL + 33.0 * Q2YL) * Longitud / 120.0;
                double dx = Longitud / (NumeroPuntos - 1);
                for (int i = 0; i < NumeroPuntos; i++)
                {
                    double x = i * dx;
                    double N = -Xa - Q1XL * x - (Q2XL - Q1XL) * x * x / (2.0 * Longitud);
                    double V = -Ra - Q1YL * x - (Q2YL - Q1YL) * x * x / (2.0 * Longitud);
                    double M = Ra * x - Ma + Q1YL * x * x / 2.0 + (Q2YL - Q1YL) * x * x * x / (6.0 * Longitud);
